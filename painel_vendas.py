...
<style>
    ...
    /* Identificador mais específico pode ser necessário se houver outras barras */
    div[data-testid="stVerticalBlock"] div.stProgress > div > div > div > div {
...
if not metas_df_raw.empty and not vendas_df_raw.empty:
    ...
    try:
        ...
        # Converter colunas de data para datetime (CORRIGIDO - SEM ESCAPE)
        ...
        # Converter colunas numéricas (tratando possíveis erros e vírgulas) (CORRIGIDO - SEM ESCAPE)
        ...
        # Converter colunas de Ano e Mês para numérico em Metas (CORRIGIDO - SEM ESCAPE)
    ...
    # --- Lógica Principal --- 
    hoje = datetime.now() # Obter datetime completo para comparação
    ...
    if metas_mes_atual.empty:
        ...
        with col2:
            # Espaço para alinhar a barra com as métricas
            st.write("") 
            st.write("")
        ...
        # CORREÇÃO DO ERRO: Comparar datetime com datetime
        ...
        if not semana_atual_df.empty:
            ...
            with col2_sem:
                ...
                # st.write(f"{progresso_sem:.1f}% atingido") # Removido para visual mais limpo
        else:
        ...
        for index, semana_info in metas_mes_atual.sort_values(by="Semana").iterrows():
            ...
            with col1_ts:
                # Usando st.metric para consistência visual
            ...
            with col2_ts:
                ...
                st.progress(min(progresso_sem / 100, 1.0))
            # st.write("---") # Removido divisor interno para visual mais limpo
...
# --- Rodapé ---
st.caption("Desenvolvido por Manus (vFinal - Revisado)")
